 ...
 */
static void sht3x(int argc, char *argv[])
{
...
static struct rt_device n32g43x_sht3x_dev;

rt_err_t sht3x_heater_power(sht3x_device_t hdev, rt_uint8_t power){
	if (power == RT_SHT3X_HEATER_ON){
		return sht3x_enable_heater(hdev);
	}
	else if (power == RT_SHT3X_HEATER_OFF){
		return sht3x_disable_heater(hdev);
	}
	return -RT_ERROR;
}

rt_err_t sht3x_read_state(sht3x_device_t hdev, void *buf){
	rt_err_t ret;
	sht3x_status_t state_ack = (sht3x_status_t)buf;
	ret = sht3x_read_status(hdev);
	if(ret==RT_EOK){
		*state_ack = hdev->status;
	}
	return ret;
}
rt_err_t sht3x_read_data(sht3x_device_t hdev, void *buf){
	rt_err_t ret;
	sth3x_data_t data = (sth3x_data_t)buf;
	ret = sht3x_read_singleshot(hdev);
	if(ret==RT_EOK){
		data->humidity_value = (rt_int32_t)(hdev->humidity*10.0);
		data->temperature_value = (rt_int32_t)(hdev->temperature*10.0);
		data->fetch_time = rt_tick_get();
	}
	return ret;
}
static rt_err_t n3243x_sht3x_control(rt_device_t dev, int cmd, void *args){
		rt_err_t result = RT_EOK;
		sht3x_device_t sht3x_dev = dev->user_data;
		switch (cmd)
    {
			case RT_SHT3X_CTRL_RESET:
				result = sht3x_softreset(sht3x_dev);
				break;
			case RT_SHT3X_CTRL_HEATER:
				result = sht3x_heater_power(sht3x_dev, (rt_uint32_t)args & 0xff);
				break;
			case RT_SHT3X_CTRL_READ_STATE:
				result = sht3x_read_state(sht3x_dev,args);
				break;
			case RT_SHT3X_CTRL_READ_DATA:
				result = sht3x_read_data(sht3x_dev,args);
				break;
			default:
				result = -RT_ERROR;
				break;
...
const static struct rt_device_ops soft_sht3x_ops = 
{
    RT_NULL,
    RT_NULL,
    RT_NULL,
...
int sht3x_port(void)
{
	...
	n32g43x_sht3x_dev.type = RT_Device_Class_Unknown;
#ifdef RT_USING_DEVICE_OPS
	n32g43x_sht3x_dev.ops     = &soft_sht3x_ops;
#else
	n32g43x_sht3x_dev.init    = RT_NULL;
	n32g43x_sht3x_dev.open    = RT_NULL;
	n32g43x_sht3x_dev.close   = RT_NULL;
	n32g43x_sht3x_dev.read    = RT_NULL;
	n32g43x_sht3x_dev.write   = RT_NULL;
	n32g43x_sht3x_dev.control = n3243x_sht3x_control;
#endif

	dev = sht3x_init("i2c1", USING_SHT3X_ADDR);
	n32g43x_sht3x_dev.user_data = dev;
	
	rt_device_register(&n32g43x_sht3x_dev, "sht3x", RT_DEVICE_FLAG_RDWR);
	return 0;
}
INIT_DEVICE_EXPORT(sht3x_port);
