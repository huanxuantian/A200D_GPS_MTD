...
int modbus_rt_mutex_trylock(modbus_rt_mutex_t* m) {
    return rt_mutex_take((m->mutex), RT_WAITING_NO);
}
...
int modbus_rt_sem_wait_timeout(modbus_rt_sem_t* m,int mstimeout) {
    rt_tick_t tick;
    tick = rt_tick_from_millisecond(mstimeout);
    return rt_sem_take(m->sem, tick);
}
